...
class Decoder():
    def __init__(self, args):
        ...
        if 'llama' in args.model.lower():
            self.model,self.tokenizer = get_model() # You need to change here.
            self.tokenizer.pad_token = self.tokenizer.eos_token

    def decode(self, args, input):

        if 'llama' in args.model.lower():
            input_ids = [self.tokenizer(i).input_ids for i in input]
            output = self.model.generate(input,max_new_tokens=450,top_k=0)
            output = [self.tokenizer.batch_decode(output[o:o+1,:][:,len(input_ids[o])-1:])[0] for o in range(len(output))]
            response = [i.split('</s>')[0].split('Q:')[0].split('Q.')[0].split('\nQ')[0].replace('<unk>', '').replace('<pad>', '').replace('</s>', '') for i in
                        output]
        return response
...
def create_verifier_demo_text(args, cot_flag):
    ...
    if args.dataset in ("multiarith", "gsm8k", "addsub", "svamp", "singleeq"):
        ...
        x.append(
            "\"It is 鈥橷'/19/1969 today. 24 hours later is 04/20/1969.\" What is the answer of 'X'?")
        z.append(
